...
// this script is ran before building code
 ...
 */
 class geoPhotoGalleryGenerator {
        ...
        var createMarkerDataPromises = markerData.map((marker) => {
            ...
            return new Promise((resolve, reject) => {
                try {
                    ...
                    blogImgs = this.filterImgs(blogImgs);
                    ...
                    altImgs = this.filterImgs(altImgs);  
     ...
     */
    filterImgs(imgs) {
        const regex = RegExp('introImg.jpg|.DS_Store|entryStamp.png|(--(sm|md|lg).jpg$)');
    ...
    collectPhotos(markerObj, path) {
        try {
            ...
            console.log(`Unable to collect photo data at ${path}${markerObj.id}.  This may not be an error.`);
            return [];
        }
    }
    ...
    getCoordinates(markerObj) {
        ...
        fetch(`${googleMapsLocationURL}${markerObj.location}&key=${process.env.GOOGLE_MAPS_API_KEY}`)
            ...
            .then(json => {
                ...
                if(status && status == 'OK') {
                    markerObj.geometry.coordinates = [results[0].geometry.location.lng, results[0].geometry.location.lat];
    ...
    writeFile(fileContents) {
        const moduleExport = `export default {markerData: ${JSON.stringify(fileContents)}};`;
